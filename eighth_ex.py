...
def replace_with_0_array_values(array_to_replace, number_to_compare) -> None:
    """
    ...
    """
    print(np.where(array_to_replace == number_to_compare, 0, array_to_replace))
    print(np.where(array_to_replace < number_to_compare, 0, array_to_replace))
    print(np.where(array_to_replace > number_to_compare, 0, array_to_replace))


if __name__ == '__main__':
    replace_with_0_array_values(np.array([[9.54, 8.38, 5.9549], [3.6, 8.32, 6.99], [1.5, 22.2, 10.23]]), 8.32)
